...
/*****************************************
	selectBox : ����Ʈ�ڽ� ������(select box plugin)
...
(function($){
	...
	$.fn.sSelect = function(options) {
		...
		var defaults = {
			...
			containerStyle: '' //custom 2012-09-27 @wddo
		};
		...
		$input = $(this);
		if($input.attr('style') != undefined){opts.containerStyle = $input.attr('style');} //custom 2012-09-27 @wddo
		var $containerDivText = $('<div title="' + $input.attr('title') + '" class="selected-headline" style="' + opts.containerStyle + '"></div>'), //custom 2012-09-27 @wddo
		$containerDiv = $('<div style="' + opts.containerStyle + '" class="select-list-box ' + opts.containerClass + '"></div>'), //custom 2012-09-27 @wddo
		...
		//@wddo layout write :	select($input)
		...
		$containerDivText.data('ssReRender',!$containerDivText.is(':visible'));
			...
			//custom 2012-10-25 @wddo
			//default index 0 LI selected
			if (newLiLength != 0){
			...
			//if !ie outline none 2012-11-13 @wddo
			...
			$containerDivText.bind('click.sSelect',function(event){
				...
				$('.select-list').not($(this).next()).hide()
					.parent()
						.css('position', 'static');
						//.removeClass('select-list-selfocus'); // @wddo 2012-10-18 // ie7 list scrolling error
			...
			$newLi.bind('mouseenter.sSelect',
				function(e) {
					...
					$newLi.removeClass('select-list-selected'); // @wddo 2013-05-28 reset selected effect
			...
			function navigateList(currentIndex, init){
				$newLi.removeClass('select-list-hover'); // @wddo 2013-05-28 reset hover effect
				...
				if ($newUl.is(':visible')){
					//$newLi.eq(currentIndex).focus(); IE ������ ���Ͽ� �Ͻ� �ּ� ó��
				}

				var text = $newLi.eq(currentIndex).html();
				var val = $newLi.eq(currentIndex).parent().data('key');

				//@wddo 2013-03-25 // option if value 'http://' -> siblings A tag attribute href change
			...
			function keyPress(element) {
				...
				$(element).unbind('keydown.sSelect').bind('keydown.sSelect',function(e){
					...
					switch(keycode) {
						case 9: //tab
							//@wddo 2013-05-28 list close
							$('.select-list').not($(this).next()).hide()
								.parent()
									.css('position', 'static');
							break;
						case 40: //down
							//@wddo 2013-05-28 keyborad open
							if (e.altKey) {
			...
			$containerDiv.bind('focus.sSelect mouseover.sSelect',function(){ // @wddo 2013-03-12 // xhtml by add mouseover
				if(getDisabled())return;
				if(typeof document.attachEvent === 'undefined')$containerDivText.addClass('select-list-selfocus'); // @wddo 2012-10-18 // ie7 list scrolling error
				keyPress(this);
			});

			$containerDiv.bind('blur.sSelect mouseout.sSelect',function(){ // @wddo 2013-03-12 // xhtml by add mouseout
				if(getDisabled())return;
				if(typeof document.attachEvent === 'undefined')$containerDivText.removeClass('select-list-selfocus'); // @wddo 2012-10-18 // ie7 list scrolling error
			});
			...
			//custom 2012-11-13 @wddo // add disabled attribute 
			if (getDisabled()) {
